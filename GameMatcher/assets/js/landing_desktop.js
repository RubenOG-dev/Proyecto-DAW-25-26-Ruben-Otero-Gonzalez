...
async function cargarCarrusel() {
  const contenedor = $d.getElementById("games-carousel");
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      contenedor.innerHTML =
        "<p class='text-white'>Non se atoparon xogos neste momento.</p>";
    }
  } catch (error) {
    console.error("Erro cargando o carrusel:", error);
    contenedor.innerHTML =
      "<p class='text-danger'>Erro ao conectar co servidor.</p>";
  }
}
