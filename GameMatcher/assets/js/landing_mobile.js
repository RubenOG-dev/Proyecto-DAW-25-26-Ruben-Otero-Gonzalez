const $d = document,
  slider = $d.querySelector(".games-slider"),
  cards = $d.querySelectorAll(".game-card");

document.addEventListener("DOMContentLoaded", () => {
  const [prevBtn, nextBtn] = [".arrow.prev", ".arrow.next"].map((s) =>
    $d.querySelector(s),
  );
  ...
  ["load", "resize"].forEach((evt) =>
    window.addEventListener(evt, () => updatePosition(false)),
  );
});
